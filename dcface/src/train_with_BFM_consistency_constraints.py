...
LOG_ROOT = str(root.parent / 'experiments_WITH_BFM_CONSISTENCY_CONSTRAINTS')    # Bernardo
...
sys.path.append(os.path.join(os.path.dirname(__file__), 'HRN'))


def train_with_BFM_consistency_constraints(cfg: DictConfig) -> Tuple[dict, dict]:
...
@hydra.main(version_base="1.2", config_path=root / "src/configs", config_name="train_with_BFM_consistency_constraints.yaml")
def main_with_BFM_consistency_constraints(cfg: DictConfig) -> Optional[float]:
    ...
    # train the model
    metric_dict, _ = train_with_BFM_consistency_constraints(cfg)
    print(metric_dict)


if __name__ == "__main__":
    main_with_BFM_consistency_constraints()
