...
class FaceReconModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--bfm_folder', type=str, default='src/HRN/assets/3dmm_assets/BFM')
    ...
    def __init__(self, opt):
        ...
        self.bfm_UVs = np.load('src/HRN/assets/3dmm_assets/template_mesh/bfm_uvs2.npy')
        self.bfm_UVs = torch.from_numpy(self.bfm_UVs).to(torch.device('cuda')).float()

        de_retouching_path = 'src/HRN/assets/pretrained_models/de-retouching.pth'
    ...
    # Bernardo
    def predict_results_base_only_bfm_coeffs(self):
        self.facemodel_front.to(self.device)
        # predict low-frequency coefficients
        with torch.no_grad():
            output_coeff = self.net_recon(self.input_img)
            ...
            #     'trans': translations
            # }
            coeffs_dict = self.facemodel_front.split_coeff(output_coeff.detach())   # [B, 257]
