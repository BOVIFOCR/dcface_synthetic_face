...
class MeshRenderer(nn.Module):
    ...
    def render_uv_texture(self, vertex, tri, uv, uv_texture):
        ...
        # for range_mode vetex: [B*N, 4], tri: [B*M, 3], for instance_mode vetex: [B, N, 4], tri: [M, 3]
        tri = tri.type(torch.int32).contiguous()
        rast_out, _ = dr.rasterize(self.glctx, vertex_ndc.type(torch.float32).contiguous(), tri, resolution=[rsize, rsize], ranges=ranges)
        ...
        rast_out, rast_db = dr.rasterize(self.glctx, vertex_ndc.type(torch.float32).contiguous(), tri, resolution=[rsize, rsize],
                                         ranges=ranges)
